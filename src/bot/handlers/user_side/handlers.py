...
from bot.keyboards import choose_subject_keyboard, start_keyboard
...
def generate_queue(subject):
    ...
    lab_columns = " + ".join(
        [f"CASE WHEN Lab{i} = 'True' THEN 1 ELSE 0 END" for i in range(1, labs + 1)]
    )
    query = f"""
...
async def on_queue_selection(call: types.CallbackQuery):
    ...
    message_text += "\n".join(
        [
            f"{i+1}. {student[0]} - Сдано лабораторных: {student[2]}"
...
def register_handlers_user(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(
        on_normal_defense_callback, lambda call: call.data == "choose_subject"
    )
    dp.register_callback_query_handler(
        on_queue_selection, lambda call: call.data in ["OP_queue", "EVM_queue"]
    )
