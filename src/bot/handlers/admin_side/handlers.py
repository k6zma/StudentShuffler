...
from bot.keyboards import admin_panel_keyboard, subjects_keyboard, labs_keyboard, start_keyboard
...
async def on_admin_panel(call: types.CallbackQuery):
    ...
    if user_id in ADMIN_USERS:
        await call.message.answer("Вы в админской панели", reply_markup=admin_panel_keyboard())
    else:
...
async def on_edit_student(call: types.CallbackQuery):
    ...
    await call.message.answer("Выберите предмет для добавления:", reply_markup=subjects_keyboard(student_id))

async def on_subject_selected(call: types.CallbackQuery):
    _, student_id, subject = call.data.split('_')
    await call.message.answer("Выберите лабораторную работу для добавления:", reply_markup=labs_keyboard(student_id, subject, LABS_COUNT[subject]))
...
async def on_lab_selected(call: types.CallbackQuery):
    ...
    await call.answer(f"Лабораторная работа {lab_number} по предмету {subject} отмечена как сданная", show_alert=True)
...
def register_handlers_admin(dp):
    dp.register_callback_query_handler(on_admin_panel, lambda call: call.data == 'admin_configuration')
    dp.register_callback_query_handler(on_edit_student, lambda call: call.data.startswith('edit_'))
    dp.register_callback_query_handler(on_subject_selected, lambda call: call.data.startswith('subject_'))
    dp.register_callback_query_handler(on_lab_selected, lambda call: call.data.startswith('lab_'))
